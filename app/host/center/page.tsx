...
export default async function HostCenterPage() {
	...
	if (user.role === 'SELLER') {
		const flat = await prisma.flat.findMany({ where: { userId: user.id } })
		const marriageGarden = await prisma.marriageGarden.findMany({ where: { userId: user.id } })
		console.log(flat, marriageGarden)
