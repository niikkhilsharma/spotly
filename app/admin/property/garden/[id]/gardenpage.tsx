...
const MarriageGardenDetailsPage = ({ gardenData }: { gardenData: MarriageGarden }) => {
	...
	async function handleApprove({ status }: { status: string }) {
		try {
			const response = await axios.post('/api/admin/garden/approve', {
				id: gardenData.id,
				status: status,
			})
				...
				<div className="space-y-6">
					...
					<Card>
						<CardContent className="flex gap-4">
							{gardenData.status === 'pending' && (
								<>
									<Button variant="outline">Reject</Button>
									<Button onClick={() => handleApprove({ status: 'approved' })} className="bg-green-600 hover:bg-green-700">
										Approve Property
									</Button>
								</>
							)}
							{gardenData.status !== 'pending' && (
								<>
									<Button variant="outline">Reject</Button>
									<Button onClick={() => handleApprove({ status: 'rejected' })} className="bg-green-600 hover:bg-green-700">
										Reject Property
									</Button>
								</>
							)}
